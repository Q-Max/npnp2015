...
{
	int i, maxi, maxfd, listenfd, connfd, sockfd,tcpfd;
	const int on=1;
	unsigned int id = 0;
	int state = 0;
	...
	// if(setsockopt(tcpfd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) < 0) {
	// 	perror("Error");
	// }
	bzero(&servaddr, sizeof(servaddr));
	servaddr.sin_family = AF_INET;
	...
	// 		len = sizeof(servaddr);
	// 		if((n = recvfrom(tcpfd, mesg, MAXLINE, 0, (struct sockaddr *) &servaddr, &len)) < 0 ){
	// 			printf("timeout\n");
	// 		}
	// 		// mesg[0] = ACK:
	// 		// sendto(tcpfd, mesg, 1, 0, (struct sockaddr *) &servaddr, len);
	...
	// 			strcpy(mesg, WELCOME);
	// 			sendto(tcpfd, mesg, strlen(mesg), 0, (struct sockaddr *) &servaddr, len);
	// 			continue;
	// 		}
	...
	// 			continue;
	// 		}
	// 		sendto(tcpfd, mesg, n, 0, (struct sockaddr *) &servaddr, len);
	// 	}
	// }
}
int max(int num1, int num2) 
...
void str_echo(int tcpfd)
{
ssize_t n;
char buf[MAXLINE]; //MAXLINE is defined by user
again:
while ( (n = read(tcpfd, buf, MAXLINE)) > 0) {
	len = sizeof(servaddr);
	if((n = recvfrom(tcpfd, mesg, MAXLINE, 0, (struct sockaddr *) &servaddr, &len)) < 0 ){
		printf("timeout\n");
	}
